...
def shortest_path(source, target):
    ...
    while True:
        ...
        for state in neighbors_for_person(currNode.state, target):
            if not frontier.contains_state(state) and state not in visited:
                frontier.add(Node(state=state[1], parent=currNode, action=state[0]))
...
def neighbors_for_person(person_id, target):
    """
